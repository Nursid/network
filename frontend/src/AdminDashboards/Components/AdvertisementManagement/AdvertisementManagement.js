...
const AdvertisementManagement = () => {
  ...
  const getTypeBadge = (type) => {
    ...
    return <span className={`badge bg-${variants[type] || 'secondary'}`}>{type}</span>;
  };
        ...
        >
          ...
          >
            <div className={`d-flex align-items-center ${isMobile ? 'flex-column' : 'justify-content-between'} p-${isMobile ? '3' : '4'}`}>
              ...
              <div className="d-flex gap-2">
                ...
                >
                  <span>➕</span>
                  {isMobile ? 'Add Ad' : 'Add Advertisement'}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className={`flex-grow-1 ${isMobile ? 'px-2' : 'px-4'} pb-4`} style={{ overflow: 'hidden' }}>
            <div style={{ 
              ...
              <AdminDataTable 
                rows={DataWithID(advertisements)} 
